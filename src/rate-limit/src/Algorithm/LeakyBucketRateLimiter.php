...
class LeakyBucketRateLimiter implements RateLimiterInterface
{
    ...
    public function attempt(string $key, int $maxAttempts, int $decay): bool
    {
        ...
        $result = $this->redis->eval(
            ...
            [$this->getKey($key), $maxAttempts, $leakRate, microtime(true)],
            1
        );
    ...
    {
        $water = $this->redis->hGet($this->getKey($key), 'water');
        return $water ? (int) $water : 0;
    }

    public function remaining(string $key, int $maxAttempts): int
    {
        $water = $this->attempts($key);
        return max(0, $maxAttempts - $water);
    }
