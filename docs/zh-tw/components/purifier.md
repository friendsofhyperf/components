...
將自動發現服務提供商。您不需要在任何地方新增 Provider.

## 用法

在請求或中介軟體中使用以下方法，以清理 HTML:
...
或者，如果您想在資料庫模型中清理 HTML，可以使用我們自定義的 casts:
...
{
    protected $casts = [
        'bio'            => CleanHtml::class, // 在獲取和設定值時都會進行清理
        'description'    => CleanHtmlInput::class, // 設定值時清理
...
配置檔案 `config/autoload/purifier.php` 內容如下：
