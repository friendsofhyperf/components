...
Model Scope 元件透過可重複使用的 `#[ScopedBy]` 註解註冊 Hyperf 模型全域
Scope。
...
元件的 `ConfigProvider` 會自動註冊 Scope 監聽器，無需發布設定檔。

## 定義 Scope

每個 Scope 都必須實作 `Hyperf\Database\Model\Scope`。
...
`classes` 參數可以接收單一 Scope 類別或 Scope 類別陣列，該註解也可以重複
使用。`priority` 預設為 `0`；優先順序越高的 Scope 越先註冊。同一個註解中
傳入的所有類別共用一個優先順序。
...
觸發 `BootApplication` 時，元件會讀取所有帶有 `#[ScopedBy]` 註解的模型類別。
對於每個宣告的 Scope 類別，元件會：

1. 驗證類別存在並實作 `Hyperf\Database\Model\Scope`；
2. 檢查容器中是否存在該類別；
