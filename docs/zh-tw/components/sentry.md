...
可選整合需要安裝對應的 Hyperf 或用戶端套件，例如 `hyperf/amqp`、`hyperf/crontab`、
...
該命令會釋出 `config/autoload/sentry.php`。核心配置如下：
...
配置檔還支援 Sentry SDK 選項，例如 `server_name`、`traces_sampler`、
...
元件預設啟用鏈路追蹤，並可以在 HTTP、RPC、佇列、Kafka、AMQP、排程任務、命令和
...
預設指標涵蓋請求耗時、協程伺服器統計、命令耗時、資料庫和 Redis 連線池，以及存在
相關套件時的非同步佇列。`#[Counter]` 和 `#[Histogram]` 可新增方法級自訂指標。
...
預設綁定使用 `FriendsOfHyperf\Sentry\Transport\CoHttpTransport`。使用下面的環境
變數調整佇列和併發：
