...
Confd 元件從 Etcd 或 Nacos 取得設定，將遠端值映射為環境變數名稱，並可將其寫入
已有的 `.env` 檔案或監聽變更。
...
Etcd 和 Nacos 是選用驅動相依套件。Nacos v2 gRPC API 還需要 `google/protobuf`、
`hyperf/grpc` 和 `hyperf/http2-client`。解碼 YAML 值需要 PHP YAML 擴充功能。
...
| `drivers.<name>.mapping` | 將遠端設定路徑映射到環境變數名稱。 |
...
從選定驅動取得映射值，並更新設定的已有 `.env` 檔案：

```shell
php bin/hyperf.php confd:env
```

使用 `--env-path`（或 `-E`）覆寫 `confd.env_path`：
