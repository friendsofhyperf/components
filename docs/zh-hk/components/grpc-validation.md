...
此組件面向 Hyperf 3.2，並依賴 `hyperf/context`、`hyperf/di`、`hyperf/grpc-server`
...
範例中的 `HiUser` 和 `HiReply` 是專案根據 `.proto` 定義產生的類別。組件會驗證傳給已註解方法的
...
| `resolve` | `bool` | `true` | 為 `true` 時，驗證失敗會立即拋出 gRPC 驗證例外。 |

## 使用 Form Request

需要重用規則時可傳入 Form Request 類別。該類別必須能從容器解析：
...
- 使用 `resolve: false` 時，組件會建立並儲存驗證器，但不會自動檢查它或在失敗時拋出例外。
