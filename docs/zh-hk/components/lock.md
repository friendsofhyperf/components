...
執行發佈的遷移前，請將其中的 `value` 欄位改為 `owner`。目前 `DatabaseLock` 實作讀寫的是
`owner` 欄位。
...
`LockFactory::make()` 的第四個參數是配置名稱，而不是驅動類別名稱。

所選的 `lock.<driver>` 配置不存在時，工廠會拋出 `InvalidArgumentException`。你也可以新增
自訂配置，其驅動須實作 `FriendsOfHyperf\Lock\Driver\LockInterface`。
...
不傳名稱呼叫 `lock()` 時返回 `LockFactory`，否則返回 `LockInterface` 實例。參數如下：

- `name`：鎖名稱。
- `seconds`：請求的鎖有效期秒數，預設為 `0`。
- `owner`：可選的擁有者識別；省略時會隨機產生。
...
`get()` 只嘗試取得一次；未傳回呼時返回布林值。傳入回呼時，它會返回回呼結果，並在
`finally` 區塊中釋放鎖：
...
`block()` 會重試，直到取得鎖或達到等待時限；逾時時拋出 `LockTimeoutException`。傳入回呼
時也會自動釋放鎖：
...
`getRemainingLifetime()` 返回鎖實例追蹤的過期狀態。
...
鎖，然後在方法返回或拋出例外後自動釋放。`ttl` 是鎖有效期，`driver` 用於選擇鎖配置。
