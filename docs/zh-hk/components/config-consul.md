...
該套件會自動安裝所需的 Hyperf 配置中心、Consul、編解碼和字串處理依賴。
...
| `drivers.consul.client` | 可選的 Consul 用戶端配置，支援 `uri` 和 `token`。 |
| `drivers.consul.namespaces` | 要遞迴拉取的 Consul KV 前綴陣列。 |
| `drivers.consul.mapping` | 將規範化後的 Consul 鍵映射到 Hyperf 配置鍵；只有已映射的鍵會被套用。 |
| `drivers.consul.interval` | 拉取間隔，單位為秒，預設為 `5`。 |

省略 `client` 或將其設為空時，元件會重用 Hyperf 容器中綁定的
`Hyperf\Consul\KVInterface` 用戶端。配置 `client` 時，`uri` 預設為
