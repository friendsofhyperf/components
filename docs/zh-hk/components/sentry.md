...
可選集成需要安裝對應的 Hyperf 或客户端包，例如 `hyperf/amqp`、`hyperf/crontab`、
...
配置文件還支持 Sentry SDK 選項，例如 `server_name`、`traces_sampler`、
...
`Graceful` 支持 `swallow`、`rethrow`、`fallback` 和 `translate` 四種策略。

## 鏈路追蹤

組件默認啓用鏈路追蹤，並可以在 HTTP、RPC、隊列、Kafka、AMQP、定時任務、命令和
